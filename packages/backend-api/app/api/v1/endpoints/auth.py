...
async def register(payload: UserRegisterSchema, db: AsyncIOMotorDatabase = Depends(get_optional_db)):
...
async def login(payload: UserLoginSchema, db: AsyncIOMotorDatabase = Depends(get_optional_db)):
